...
class colorbar:
    ...
    def set_title(self, title, size):
        ...
        self.scalar_bar.GetTitleTextProperty().SetFontSize(size)

    def set_label(self, nlabels, size):
        self.scalar_bar.SetNumberOfLabels(nlabels)
        self.scalar_bar.GetLabelTextProperty().SetFontSize(size)

    def set_position(self, pos):
        self.scalar_bar.SetPosition(pos[0], pos[1])

    def set_size(self, width, height):
        self.scalar_bar.SetMaximumWidthInPixels(width)
        self.scalar_bar.SetMaximumHeightInPixels(height)
